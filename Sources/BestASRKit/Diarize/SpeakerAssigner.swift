...
public enum SpeakerAssigner {
    ...
    /// Zero overlap → nil (unknown, never fabricated). Ties → the
    /// earlier-starting turn. Labels are `SPEAKER_1`-based ordinals in order
    /// of first appearance across the returned assignments.
            ...
            for turn in turns {
                ...
                if let current = best {
                    if overlap > current.overlap
                        || (overlap == current.overlap && turn.start < current.turn.start)
                    {
                        best = (turn, overlap)
                    }
                } else {
