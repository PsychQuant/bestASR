...
> **原本列在此處的第二項已刪除並實作。** 它寫的是「`Router` 排除身分不完整的候選（→ #187）……canonical 化後 store 內已無 `.unknown` 的量測，所以這條規則現在沒有可觸發的資料」。**實測推翻該理由**：本機 store 的 383 筆量測中有 **44 筆**（四個 mlx-audio key）canonical 化後 quantization 仍是 `unknown`。規則已於任務 3.3 落地——**但 round 9 又證明它落地的方向是反的**：它排掉的 44 筆全部帶 commit sha pin，放行的 47 筆量測當時精度由相依套件預設決定。根因是判準問今天的目錄而不是記錄自身（design D9）。現改為**排序並具名**，證據取自 `MeasurementRow.hfRevision`（#16 早已為同一理由存在）。這兩次都是同一個毛病：**寫下判準時沒有拿資料去對**。
