...
**Machine-independence 的誠實邊界（verify 補充 2026-07-05）**：「文字比對是 machine-independent」為真；「模型輸出文字跨機器 bit-identical」是另一個未證立的前提——CoreML 推論跨晶片世代 / compute unit / OS 版本不保證位元相同，greedy decode 可把 decision boundary 上的 logits 微差放大成不同 token。同機重跑 ±0.0000 已實證；跨機不變性是 tolerance（±0.02）預期吸收的範圍，非證明。播種溯源（機器、模型 repo revision）記在 `benchmarks/baseline-meta.json`。

**同機隨機性的實測與對策（verify live 發現 2026-07-06）**：驗證期間 live 觀測到 cv-zhtw-4 在同機、同模型、同音檔下 CER 0.2097 ↔ 0.1452 漂移——機制是 Whisper 的 **temperature fallback**（品質門檻觸發時以 temperature > 0 重解碼 = 隨機取樣；WhisperKit 預設 fallbackCount=5）。受控 A/B 釐清方向：**第一遍 greedy = 0.1452（加旗標 3/3 精確重現）**；預設 fallback 路徑通常把它重解碼成**更差的 0.2097**、偶爾落回 0.1452——「救援」在此例反而傷分且不穩定。對策：gate 的 canary 改用 `bestasr benchmark --decode-deterministic`（WhisperKit `temperatureFallbackCount=0`、whisper.cpp `-nf`）保留第一遍 greedy——**canary 要的是可重現性，偶發救援留給正常 transcribe 路徑**。cv-zhtw-4 golden 依此重播種 0.2097 → 0.1452（舊值是 fallback 路徑產物）；其餘 11 corpus 從不觸發 fallback，golden 不變。
...
- **gate fail 的第三種原因**：reference model 的上游 artifact（HF argmaxinc/whisperkit-coreml）更新會位移輸出——gate 失敗要對照 model provenance triage（code 退步、語料變動、還是上游模型換版），不可直接歸因 code。緩解（落地形式 2026-07-05）：baseline 條目 `model` 欄記模型名；revision 錨點另記於 `benchmarks/baseline-meta.json`（播種當日的 HF repo revision + 播種機器 + 誠實註記——audit anchor 而非 cryptographic pin，因 WhisperKit 於下載時解析 `main`、本機 CoreML cache 未重 hash），gate 報錯訊息提示三種可能原因。
