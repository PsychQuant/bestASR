...
#26（兩階段裁決之二，#25 已 ship）：diarization 只給匿名 SPEAKER_N；會議/上課場景要的是「這是誰」。FluidAudio v0.15.4 每 turn 帶 embedding。**設計修正（apply 中實測）**：SDK 的 `initializeKnownSpeakers` 預載路徑在 DiarizerManager pipeline 不進 clustering 決策（預載者仍 distance=inf），故 identification 改走**自控 post-hoc cosine 比對**（純函式 `SpeakerIdentifier.resolve`，可完全單元測試、不賭 SDK 內部行為）。
...
3. `DiarizationEngine.diarize(audioPath:)` 回 `DiarizationOutput{turns, embeddings}`；`SpeakerIdentifier.resolve` 後製 cosine 比對映射 raw id → 註冊名
