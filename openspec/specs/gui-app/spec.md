...
The human-facing macOS surface of bestASR: a SwiftUI GUI app shipped as a
signed, notarized, stapled dual-track `bestASR.app` bundle that also carries
the `bestasr-mcp` helper and the CLI (`bestasr-cli`), so one artifact serves
people (drag a file → transcript) and agents (bundle-path MCP) alike.
