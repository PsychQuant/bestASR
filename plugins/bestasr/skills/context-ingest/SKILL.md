---
name: context-ingest
description: context pipeline 第一步（轉錄前）：把任意格式的領域文件（pdf/docx/txt/md/圖片…）蒸餾成 bestASR 的 context.json（terms/names/phrases），寫入 context 資料夾供轉錄時的 context biasing（top-down / prompt biasing：以領域知識偏引辨識）與事後校對（srt-proofread）共用。當使用者提到「整理 context」「把文件變成 context」「準備 ASR 術語」「ingest 文件」「更新 bestasr context」「top-down」「topdown」「context biasing」「術語偏引」「讓專有名詞/人名辨識正確」時使用。
---
...
> **絕不建成 `./bestasr-context/`（#107 breaking）**：舊 convention 已移除、新版讀不到。若目錄已有 pre-#107 的 `bestasr-context/`，先 `git mv bestasr-context .bestasr/context` 遷移再用，不要並存。
