---
name: transcript
description: 把任意來源轉成 SRT 逐字稿——貼 YouTube（或任何 yt-dlp 支援的影音站）網址、本地音／視訊檔路徑、或現成字幕檔，skill 抓下來並用 bestASR ASR 轉錄。同時是 context pipeline 的中樞：轉錄前主動偵測領域文件（建議 context-ingest）、轉錄中自動帶 context biasing（top-down／topdown 術語偏引）、轉錄後建議 srt-proofread 沿用同一份 context。沒給來源就主動問。當使用者說「幫我轉錄這個」「做逐字稿」「這個影片／音檔轉字幕」「這個 YouTube 轉逐字稿」「下載並轉錄」並附上一個來源時使用。注意：這是「下載音訊 + bestASR 自己 ASR 轉」，品質由 bestASR 模型決定；若使用者要的是「抓 YouTube 上已有的官方／自動字幕」而不是重新辨識，那是 yt-subtitle-downloader 的工作，不要用本 skill。
---
...
| **Plaud 錄音** | 使用者指名一筆 Plaud 錄音（名稱／日期／內容關鍵字） | 交給 `plaud-srt` 產 SRT（**不 ASR**）——Plaud 已有逐字稿，重跑等於用較差的音訊再辨識一次。需要 plaud-mcp-connector，見下方三層缺席判定 |
...
### 3b. Plaud 錄音（不 ASR，交給 plaud-srt）

Plaud 的錄音**已經有逐字稿**。對它重跑 ASR 是拿較差的音訊再辨識一次——同
「現成字幕」那一列的道理，來源已經是文字了。

**取回路徑：呼叫 `plaud-srt` skill，不要自己讀那個 plugin 的 cache。**

cache 就在磁碟上（`~/.plaud-connector/cache`，可用 `PLAUD_CACHE_DIR` 覆寫），
直接讀看起來更省事。不要這麼做：那份 cache 的**行格式**是該 plugin 的內部
約定，而它已經因為「兩個生產者各寫各的」壞過一次
（`PsychQuant/plaud-mcp-connector#40`：CLI 路徑寫時間**區間**、消費端只認
時間**點**，於是被 README 推薦的那條路一律產出 0 個 cue，全套件綠了數個月）。
多一個直讀它的消費者，就是把同一個缺陷複製到第二個 repo，而且是跨 repo 的
版本——那邊改格式時這邊不會轉紅。

流程：

1. 使用者指名一筆錄音（名稱／日期／內容關鍵字）。找不到唯一一筆就問，不要猜。
2. 交給 `plaud-srt` 產出 `.srt`。
3. 拿到路徑後，回到步驟 3「現成字幕」那一列處理（它已經是 SRT，時間軸忠實
   保留，不重編）。

**回報時要說清楚這條路沒有跑 ASR** ——「用了哪個 backend/model」在這裡不適用，
說成有跑等於把別人的辨識結果報成 bestASR 的。

#### 三層缺席判定（缺哪一層，訊息就不一樣）

bestASR 本身零外部依賴，這是第一個跨 plugin 的來源。缺席不是錯誤，是「這條
路今天走不了」，所以每一層都要能講出**使用者下一步做什麼**：

| 層 | 怎麼判定 | 缺席時說什麼 |
|---|---|---|
| **plugin** | `plaud-srt` / `plaud-index` skill 不在可用清單 | 「這需要 plaud-mcp-connector plugin，本機沒裝。」給安裝來源，然後**回到一般來源**——請對方給音檔或網址 |
| **帳號** | 有 plugin，但回報未授權 | 「plugin 在，但 Plaud 還沒登入。」**那邊有兩套獨立的 token**（官方 CLI 用 `~/.plaud/tokens.json`、MCP 用 `~/.plaud/tokens-mcp.json`），所以「MCP 能用但 CLI 說未授權」是正常的、不是 bug。把錯誤訊息**原文**轉給使用者，讓對方知道要登的是哪一套，不要只說「請登入」 |
| **這一筆** | 有 plugin、有登入，但該錄音沒有逐字稿 | 「這筆錄音在 Plaud 上還沒有逐字稿。」**Plaud 的 API 分不出『從未請求』『進行中』『失敗』三種狀態**，而需要行動的是第一種：請對方在 Plaud 開啟該錄音按 產生 / Generate。若只是想現在就要，可以改給音檔走一般 ASR 路徑 |

三層都要**先講缺什麼、再講怎麼辦**。只說「找不到 Plaud」會讓人以為是自己
打錯名字。
...
1. **轉錄前——主動偵測**：拿到音訊來源後，先看兩件事：
   ...
   - **沒有 context 但音訊同目錄或 cwd 有領域文件**（pdf / docx / pptx / csv / md /
     txt 講義、投影片、名單、術語表）→ 用 AskUserQuestion 提示：「發現這些文件，
...
**一路走完模式**：使用者一開始就表達「全自動」「一路做到好」「幫我
ingest＋轉錄＋校對」時，**不逐步停問**——ingest → 轉錄 → proofread 三步
直通到底，只在最後交付時彙報每步做了什麼（含 --explain 的注入清單與
proofread 的 per-cue diff）。逐步提示是預設；一路走完是使用者一句話就能
切換的模式。
